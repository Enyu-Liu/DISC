...
def vox2pix(cam_E, cam_K, vol_origin, vox_size, img_shape, scene_size, post_rots, post_trans):
    """
    ...
    Args:
        ...
        post_rots: rotation matrix of img aug
        post_trans: trans matrix
    ...
    cam_pts = fusion.TSDFVolume.vox2world(vol_origin, vox_coords, vox_size)  # voxel coor(index format) to world coor(float format in meters)
    cam_pts = fusion.rigid_transform(cam_pts, cam_E) # world coor to camera coor(frustum) with extrinsic parameters

    # Project camera coordinates to pixel positions
    projected_pix = fusion.TSDFVolume.cam2pix(cam_pts, cam_K) # camera coor to pixel coor  #(num_points, 2)
    
    # TODO: add the same augmentation as input image
    # resize and crop -> aug mat     M = aug_mat@resize_crop@flip@rot
    # shape: (1, 2, 2) @ (n, 2, 1) -> (n, 2, 1)
    projected_pix = np.matmul(post_rots[:2, :2][np.newaxis, ...], projected_pix[..., np.newaxis])  
    projected_pix = np.squeeze(projected_pix, axis=-1)
    projected_pix = projected_pix + post_trans[:2][np.newaxis, ...]
