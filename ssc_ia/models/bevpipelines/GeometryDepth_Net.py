...
class GeometryDepthNet(BaseModule):
    def __init__(
        self,
        downsample=8,
        numC_input=512,
        ...
        **kwargs,
        ...
        super(GeometryDepthNet, self).__init__()
    ...
    def forward(self, input, stereo_depth):
        ...
        mono_digit = x[:, :self.D, ...] # (1, 112, 48, 160)
        mono_volume = self.get_depth_dist(mono_digit) #  Softmax (1, 112, 48, 160)
        img_feat = x[:,  self.D:self.D + self.numC_Trans, ...]
        
        return img_feat.view(B, N, -1, H, W), mono_volume, mono_digit # (1, 112, 48, 160)