...
from .BEVFusion.bevfusion.ops import bev_pool
...
class ViewTransformerLSS(BaseModule):
    def __init__(
        self, grid_config=None, input_size=None, downsample=8,**kwargs,
    ):
        super().__init__()
    ...
    def forward(self, feat, depth_prob, cam_params, bda=None):
        B, N, C, H, W = feat.shape
        rots, trans, intrins, post_rots, post_trans = cam_params
        if bda == None:
            bda = torch.eye(3).to(rots).view(1, 3, 3).repeat(B, 1, 1)
