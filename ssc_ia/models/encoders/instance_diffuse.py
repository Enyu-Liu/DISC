...
class InstanceDiffuser(nn.Module):
    def __init__(self,
                 output_shape,
                 in_channels=128,
        ...
        super(InstanceDiffuser, self).__init__()
    ...
    def forward(self, ins_feature, ref3ds):
        """
        Upsample -> BasicBlock -> Completetion/agreBlock -> [skip1, skip2, skip3]
        Args:
            ins_feature : (bs, N, C)
            ref3ds : (bs, N, 3)
        Return:
            Multi-Scale Sparse feature, each in spatial shape of:
            (256, 256, 32) -> (128, 128, 16) -> (64, 64, 8)
            [skip1, skip2, skip3] Sponv format feature
        """
        # NOTE: Upsample Here - > Spatial shape (256, 256, 32)
        # Convert to Sparse format 
        bs, N, C = ins_feature.shape
        assert bs == 1 # Only deal with batch_size = 1 
        ins_feature = ins_feature.reshape(-1, C) #(N, C)
        ref3ds = ref3ds.reshape(-1, 3) * 2 #(N, 3) upsample
        batch_indices = torch.zeros((ref3ds.shape[0], 1)).to(ref3ds) 
        ref3ds = torch.cat([batch_indices, ref3ds], dim=1)  # (N, 4)
        input = spconv.SparseConvTensor(ins_feature, ref3ds.int(), self.sparse_shape * 2, 1) # bs = 1

        # BasicBlock
        input = self.resBlock0(input)  # 522
        
        # Completetion and Agregation
        input = self.complBlock1(input)   # 76521
        down1, skip1 = self.aggreBlock1(input) # 12860 76521
        down1 = self.complBlock2(down1) # 57514
        down2, skip2 = self.aggreBlock2(down1) 
        down2 = self.complBlock3(down2) 
        skip3 = self.aggreBlock3(down2) 

        output = [skip1, skip2, skip3]
