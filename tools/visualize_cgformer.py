...
def main(config: DictConfig):
    ...
    output_dir = osp.join(base_output_dir, 'cgformer', 'figures')
    os.makedirs(output_dir, exist_ok=True)

    for file in track(files):  # ******.pkl
        ...
        pred = outputs['pred'] if 'pred' in outputs else outputs['y_pred']  #(256, 256, 32) unit16
        target = outputs['target']

        # replace pred with CGFormer pred Here 
        # read Here
        file_name = os.path.splitext(os.path.basename(file))[0]
        cgformer_label_path = os.path.join("/mnt/A/hust_ley/CODE_LEY/Project/test_project/CGFormer-main/pred/sequences/08/predictions",\
                                            f"{file_name}.label")
        # shape (256, 256, 32) type: uint16
        cg_pred = np.fromfile(cgformer_label_path, dtype=np.uint16)
        cg_pred = cg_pred.reshape(256, 256, 32)
        cg_pred = cg_pred.astype(np.uint16)
        pred = cg_pred
